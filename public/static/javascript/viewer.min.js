...
Destroyable.prototype.collides=function(a,b){if(!this.props.sprite.destroyable||this.props.destroyedColorIndex||this.props.x<-1*this.props.frameWidth||this.props.x>canvas.width)return!1;var c=Math.floor(this.props.x),d=Math.floor(this.props.x+this.props.frameWidth),e=Math.floor(this.props.y),f=Math.floor(this.props.y+this.props.frameHeight),g=Math.floor(this.props.z-20),k=Math.floor(this.props.z+20),h;for(h in a){var l=Math.floor(0.3*a[h].props.z),n=Math.floor(0.3*a[h].props.z),m=Math.floor(a[h].props.x+
n+100),n=Math.floor(a[h].props.x-n+100),q=Math.floor(a[h].props.y+l),l=Math.floor(a[h].props.y-l),r=Math.floor(a[h].props.z);if(r>=g&&r<=k&&m>=c&&n<=d&&q>=e&&l<=f)return h}return!1};var Explosion=function(a,b,c,d,e){for(var f=[],g=0;360>g;g+=Math.round(45)){var k=new ExplosionParticle;k.x=a;k.y=b;k.zIndex=c++;k.scale=d;k.radius=randomFloat(15,25);k.color="rgba("+e+", .5)";k.scaleSpeed=randomFloat(2,4)*d;var h=randomFloat(60,150);k.velocityX=h*Math.cos(g*Math.PI/180);k.velocityY=h*Math.sin(g*Math.PI/180);f.push(k)}return f};
...
a.fillStyle=this.color,a.fill(),a.restore())}};var Game=function(a,b){var c=this;c.props={state:"LOADING",message:!1,levelPath:a,preloadImages:[],images:{},audio:{},world:!1,worldX:55E3,worldSpeed:10,destroyed:{},explosions:[],spriteAnimationFrame:1};$.ajaxSetup({cache:!0});$.getJSON(c.props.levelPath+"/level.json",function(a){c.props.world=a;for(var b=0;b<a.length;b++)if("undefined"!==typeof a[b].background&&-1===$.inArray(a[b].background,c.props.preloadImages)&&c.props.preloadImages.push(a[b].background),a[b].sprites.length)for(var f=0;f<a[b].sprites.length;f++){var g=
...
Game.prototype.tick=function(a,b){if("LOADING"==this.props.state)return!1;if("STARTED"==this.props.state||"LEADERBOARD"==this.props.state)this.props.worldX<1E3*this.props.world.length-a.canvas.width?this.props.worldX+=this.props.worldSpeed:this.endGame();var c=Math.ceil(a.canvas.width/1E3),d=Math.floor(this.props.worldX/1E3),e=Math.floor(this.props.worldX%1E3);this.renderBackgrounds(a,c,d,e);this.renderEntities(a,c,d,e);"STARTED"==this.props.state&&this.props.worldX>1E3*this.props.world.length-8E3&&
(this.props.state="LEADERBOARD",this.prepareLeaderboard());("ENDED"==this.props.state||"LEADERBOARD"==this.props.state)&&this.renderLeaderboard(a)};Game.prototype.renderBackgrounds=function(a,b,c,d){for(var e=0;e<=b;e++){var f=this.props.world[c+e];if("undefined"!=typeof f){var g=1E3*e-d;if(!(g>a.canvas.width)){var k=0,h=1E3;0>g&&(g=0,k=d,h=1E3-d);(f=this.getImage(f.background))&&a.drawImage(f,k,0,h,a.canvas.height,g,0,h,a.canvas.height)}}}};
Game.prototype.renderEntities=function(a,b,c,d){var e={},f=[],g=0;this.props.spriteAnimationFrame=8.9>this.props.spriteAnimationFrame?this.props.spriteAnimationFrame+0.1:1;for(var k=Math.floor(this.props.spriteAnimationFrame),h=-3;h<=b+1;h++){var l=this.props.world[c+h];if("undefined"!=typeof l)for(var n in l.sprites){g++;var m=l.sprites[n],q=this.props.destroyed[m.id]||!1,r=1E3*h-d+m.left,v=m.top,t=50*m.layer,s=g+1E3*m.layer,u=this.getImage(m.path);r<-1*u.width||r>canvas.width||("STARTED"==this.props.state&&
(m.left-=1.25*(m.layer-1)),e[s]=new Destroyable(m,u,r,v,t,q,k),f.push(s),m.destroyable&&!q&&(q=e[s].collides(players),0<q&&(t=players[q].props.color,s=$.inArray(t,playerColors)+1,this.props.destroyed[m.id]=s,m.destroylink&&(this.props.destroyed[m.destroylink]=s),0<m.score&&(players[q].updateScore(m.score),s=g+1E3*m.layer+200,u=5*parseInt(m.layer,10),r=Explosion(r-50,v- -1*(players[q].props.z/2),s,u,t),this.props.explosions=this.props.explosions.concat(r)),m.audio&&"none"!=m.audio&&this.props.audio[m.audio].volume(1).play())))}}for(var p in players)b=
1E3*players[p].props.vector[2]+900+players[p].props.playerNumber,e[b]=players[p],f.push(b),b=players[p].props.playerNumber+1500,e[b]=new PlayerShadow(players[p]),f.push(b);if(this.props.explosions.length)for(var w in this.props.explosions)p=this.props.explosions[w],p.dead||(e[p.zIndex]=p,f.push(p.zIndex));f.sort();for(h=0;h<f.length;h++)e[f[h]].draw(a)};
...
Game.prototype.prepareLeaderboard=function(){var a=0,b=1E4,c=[],d;for(d in players)players[d].lockPlayer(),players[d].props.score>a&&(a=players[d].props.score),players[d].props.score<b&&(b=players[d].props.score),c.push([players[d],players[d].props.score]);c.sort(function(a,b){return b[1]-a[1]});logGAEvent("Ended","Highest score",a);var e=0,f;for(f in players)e++;e=(600-60*e)/2;f=canvas.width/2-400;var g=0;for(d in c){g++;var k=c[d][0];k.props.endX=Math.floor(800*((k.props.score-b)/a)+f);k.props.endY=
Math.floor(g*k.props.endZ-k.props.endZ/2+e)}};Game.prototype.renderLeaderboard=function(a){var b=canvas.width/2,c;for(c in players)players[c].props.x<players[c].props.endX+10&&players[c].props.x>players[c].props.endX-10&&players[c].props.y<players[c].props.endY+10&&players[c].props.y>players[c].props.endY-10&&this.drawMessage(a,players[c].props.score,players[c].props.endX+140,players[c].props.endY+9,b,35,!1);socket.emit("game-end",{viewerId:viewerId,gameRoom:gameRoom,leaderboard:!1})};
...
Game.prototype.endGame=function(){this.props.state="ENDED";$(window).trigger("game-audio-stop");setTimeout(function(){var a=context.getImageData((canvas.width-1E3)/2,0,1E3,600),b=document.createElement("canvas"),c=b.getContext("2d");b.width=1E3;b.height=600;c.putImageData(a,0,0);c.font="80px NevisRegular";c.fillStyle="#FFFFFF";c.fillText("Multeor",650,550);a=b.toDataURL();socket.emit("store-leaderboard",{viewerId:viewerId,gameRoom:gameRoom,leaderboard:a});$(".leaderboard-container").fadeIn(1E3)},
1E3)};Game.prototype.getReady=function(){var a=this;if("WAITING"!=a.props.state)return!1;$(window).trigger("game-audio-start");a.props.state="GETREADY";socket.emit("game-get-ready",{viewerId:viewerId,gameRoom:gameRoom});setTimeout(function(){a.startGame()},4E3)};
...
Game.prototype.drawMessage=function(a,b,c,d,e,f,g){a.save();a.font="30px NevisRegular";a.fillStyle="#FFFFFF";b=b.toString().split(" ");for(var k="",h=0;h<b.length;h++){var l=k+b[h]+" ",n=a.measureText(l).width;n>e?(a.fillText(k,c,d),k=b[h]+" ",d+=f):(g&&(c=a.canvas.width/2-n/2),k=l)}a.fillText(k,c,d);a.restore()};var Player=function(a,b,c,d,e){var f=this;f.props={playerId:b,playerNumber:e,icon:!1,score:0,x:0,y:0,z:0,vector:[0,0,0],minX:60,maxX:a.canvas.width-180,minY:0,maxY:a.canvas.height,minZ:50,maxZ:150,color:d,lastMoves:[],meteorHeadAngle:45*e,locked:!1,endX:0,endY:0,endZ:60};f.props.y=e*(f.props.minZ+18);c?(a=new Image,a.crossOrigin="Anonymous",a.src=c,a.onload=function(){f.loadIcon(this)}):f.loadIcon()};
Player.prototype.draw=function(a){this.updatePosition();if(9<this.props.lastMoves.length){var b=this.props.lastMoves[0][1],c=this.props.lastMoves[6][0],d=this.props.lastMoves[6][1],e=this.props.lastMoves[7][0],f=this.props.lastMoves[7][1],g=100+this.props.lastMoves[9][0],k=this.props.lastMoves[9][1];a.lineWidth=this.props.z;var h=a.createLinearGradient(0,0,g-a.lineWidth/2,0);h.addColorStop(1,"rgba("+this.props.color+",1)");h.addColorStop(0,"rgba("+this.props.color+",0)");a.strokeStyle=h;a.lineCap=
"round";a.beginPath();a.moveTo(0,b);a.bezierCurveTo(c,d,e,f,g,k);a.stroke();this.props.icon&&(a.save(),a.translate(g,k),a.rotate(Math.PI/180*this.props.meteorHeadAngle),this.props.meteorHeadAngle++,b=Math.floor(0.6*this.props.z),c=Math.floor(0.6*this.props.z),a.drawImage(this.props.icon,Math.floor(-1*(b/2)),Math.floor(-1*(c/2)),b,c),a.restore())}};
Player.prototype.shadow=function(a){if(9<this.props.lastMoves.length){var b=-50*(this.props.z-this.props.minZ)/(this.props.maxZ-this.props.minZ),c=this.props.minZ,d=1.5*this.props.z-1.5*c;if(!(5>d)){var e=0+b,f=this.props.lastMoves[0][1]+d,g=this.props.lastMoves[6][0]+b,k=this.props.lastMoves[6][1]+d,h=this.props.lastMoves[7][0]+b,l=this.props.lastMoves[7][1]+d,b=100+this.props.lastMoves[9][0]+b,d=this.props.lastMoves[9][1]+d;a.lineWidth=c;c=a.createLinearGradient(0,0,b-a.lineWidth/2,0);c.addColorStop(1,
"rgba(36,42,48,0.1)");c.addColorStop(0,"rgba(36,42,48,0)");a.strokeStyle=c;a.lineCap="round";a.beginPath();a.moveTo(e,f);a.bezierCurveTo(g,k,h,l,b,d);a.stroke()}}};
...
Player.prototype.drawPolygon=function(a,b,c,d,e){var f=360/e*(Math.PI/180),g,k;a.beginPath();a.moveTo(Math.cos(l)*d,Math.cos(l)*d);for(var h=0;h<e;h++){var l=h*f,n=b+Math.cos(l)*d,l=c+Math.sin(l)*d;0<h?a.lineTo(n,l):(g=n,k=l);h==e-1&&a.lineTo(g,k)}a.closePath();a.fill()};var PlayerShadow=function(a){this.props={player:a.props}};
PlayerShadow.prototype.draw=function(a){if(9<this.props.player.lastMoves.length){var b=-50*(this.props.player.z-this.props.player.minZ)/(this.props.player.maxZ-this.props.player.minZ),c=this.props.player.minZ,d=1.5*this.props.player.z-1.5*c;if(!(5>d)){var e=0+b,f=this.props.player.lastMoves[0][1]+d,g=this.props.player.lastMoves[6][0]+b,k=this.props.player.lastMoves[6][1]+d,h=this.props.player.lastMoves[7][0]+b,l=this.props.player.lastMoves[7][1]+d,b=100+this.props.player.lastMoves[9][0]+b,d=this.props.player.lastMoves[9][1]+
d;a.lineWidth=c;c=a.createLinearGradient(0,0,b-a.lineWidth/2,0);c.addColorStop(1,"rgba(36,42,48,0.1)");c.addColorStop(0,"rgba(36,42,48,0)");a.strokeStyle=c;a.lineCap="round";a.beginPath();a.moveTo(e,f);a.bezierCurveTo(g,k,h,l,b,d);a.stroke()}}};profiler={context:!1,speedLog:[],frameCount:0,frameStartTime:0,fpsLog:[],paintCountLog:[],lastPaintCount:0,target:15,x:0,y:0,graphScale:0.25,start:function(a,b,c,d,e){window.mozPaintCount&&(profiler.lastPaintCount=window.mozPaintCount);profiler.context=a;profiler.x=c;profiler.y=d;profiler.graphScale=e;profiler.target=b;setTimeout(profiler.getStats,1E3)},tick:function(){var a=(new Date).getTime();profiler.frameStartTime&&profiler.speedLog.push(a-profiler.frameStartTime);profiler.frameStartTime=a;profiler.frameCount++;
