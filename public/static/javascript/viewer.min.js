...
(l.left-=1.25*(l.layer-1)),e[r]=new Destroyable(l,u,q,s,t,n,g),l.destroyable&&!n&&(n=e[r].collides(players,d),0<n&&(t=players[n].props.color,r=$.inArray(t,playerColors)+1,this.props.destroyed[l.id]=r,0<l.score&&(players[n].updateScore(l.score),r=f+1E3*l.layer+200,u=5*parseInt(l.layer,10),q=Explosion(q-50,s- -1*(players[n].props.z/2),r,u,t),this.props.explosions=this.props.explosions.concat(q)),l.audio&&"none"!=l.audio&&this.props.audio[l.audio].volume(1).play())),f++)}}for(var p in players)b=1E3*
players[p].props.vector[2]+900+players[p].props.playerNumber,e[b]=players[p],b=players[p].props.playerNumber+1500,e[b]=new Shadows(players[p]);if(this.props.explosions.length)for(var v in this.props.explosions)p=this.props.explosions[v],p.dead||(e[p.zIndex]=p);for(v in e)e[v].draw(a,d)};Game.prototype.renderText=function(a){var b=a.canvas.width/2;this.drawMessage(a,this.props.message,(a.canvas.width-b)/2,a.canvas.height/2.2,b,35,!0)};
...
d;a.lineWidth=c;c=a.createLinearGradient(0,0,b-a.lineWidth/2,0);c.addColorStop(1,"rgba(36,42,48,0.1)");c.addColorStop(0,"rgba(36,42,48,0)");a.strokeStyle=c;a.lineCap="round";a.beginPath();a.moveTo(e,f);a.bezierCurveTo(g,h,k,m,b,d);a.stroke()}}};if("undefined"===typeof io||!Modernizr.sessionstorage||!Modernizr.canvas||!Modernizr.websockets)$(".main").hide(),$("#error").show();else{var canvas=document.getElementById("canvas"),context=canvas.getContext("2d");canvas.width=window.innerWidth;canvas.height=600;context.globalAlpha=0.5;var socket=io.connect(window.location.hostname+":843"),players={},game=!1,playerColors="110,8,157 0,255,247 16,230,34 255,174,26 255,255,0 255,110,221 8,103,255 255,0,36".split(" "),playerColorsShuffled=shuffleArray(playerColors.slice(0)),
gameRoom=sessionStorage.getItem("game-room")||Math.floor(1E4*Math.random()+1E4);sessionStorage.setItem("game-room",gameRoom);var viewerId=sessionStorage.getItem("viewer-id")||Math.floor(1E4*Math.random()+1E4);sessionStorage.setItem("viewer-id",viewerId);$(document).ready(function(){var a=new Game("/levels/forest",gameRoom);socket.emit("new-viewer",{viewerId:viewerId,gameRoom:gameRoom});socket.on("game-end",function(b){a.abortGame()});socket.on("game-invalid",function(a){sessionStorage.setItem("viewer-id",
"");sessionStorage.setItem("game-room","");document.location="/";logGAEvent("Game hijacking?")});socket.on("update-game-state",function(a){var b=0,c;for(c in a.players)b++;b&&$("#gameStart").removeClass("disabled");b=0;for(c in a.players){var d=a.players[c].playerId;b++;if("undefined"==typeof players[d]){var e=playerColorsShuffled.splice(0,1)[0];players[d]=new Player(context,d,a.players[c].playerIcon,e,b);socket.emit("update-player-color",{viewerId:viewerId,gameRoom:gameRoom,playerId:d,playerColor:e})}}for(var l in players){b=
!1;for(c in a.players)a.players[c].playerId==l&&(b=!0);b||delete players[l]}});socket.on("player-update",function(a){if("undefined"==typeof players[a.pid])return!1;players[a.pid].props.vector=a.v});var b,c,d=(new Date).getTime(),e=1E3/60;(function g(h){requestAnimationFrame(g);b=(new Date).getTime();c=b-d;c>e&&(d=b-c%e,a&&a.tick(context,c))})();$(".gamecode-container").html(gameRoom);$("#gameStart").off("click").on("click",function(b){b.preventDefault();if($(this).hasClass("disabled"))return!1;hideInstructions();
a.getReady()});$("#gameReset button").off("click").on("click",function(b){b.preventDefault();a.resetGame()})})}
