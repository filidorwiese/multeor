...
function ExplosionParticle(){this.scale=1;this.y=this.x=0;this.radius=20;this.color="rgba(255,255,255,.5)";this.velocityY=this.velocityX=0;this.scaleSpeed=0.5;this.zIndex=0;this.update=function(a){this.scale-=this.scaleSpeed*a/1E3;0>this.scale&&(this.scale=0);this.x+=this.velocityX*a/1E3;this.y+=this.velocityY*a/1E3};this.draw=function(a,b,c){this.update(20);1>this.scale||(a.save(),a.translate(this.x,this.y),a.scale(this.scale,this.scale),a.beginPath(),a.arc(0,0,this.radius,0,2*Math.PI,!0),a.closePath(),
...
(m.left-=1.25*(m.layer-1)),e[s]=new Destroyable(m,u,r,v,t,q,k),f.push(s),m.destroyable&&!q&&(q=e[s].collides(players,d),0<q&&(t=players[q].props.color,s=$.inArray(t,playerColors)+1,this.props.destroyed[m.id]=s,0<m.score&&(players[q].updateScore(m.score),s=g+1E3*m.layer+200,u=5*parseInt(m.layer,10),r=Explosion(r-50,v- -1*(players[q].props.z/2),s,u,t),this.props.explosions=this.props.explosions.concat(r)),m.audio&&"none"!=m.audio&&this.props.audio[m.audio].volume(1).play())))}}for(var p in players)b=
1E3*players[p].props.vector[2]+900+players[p].props.playerNumber,e[b]=players[p],f.push(b),b=players[p].props.playerNumber+1500,e[b]=new Shadows(players[p]),f.push(b);if(this.props.explosions.length)for(var w in this.props.explosions)p=this.props.explosions[w],p.dead||(e[p.zIndex]=p,f.push(p.zIndex));f.sort();for(h=0;h<f.length;h++)e[f[h]].draw(a,d)};Game.prototype.renderText=function(a){var b=a.canvas.width/2;this.drawMessage(a,this.props.message,(a.canvas.width-b)/2,a.canvas.height/2.2,b,35,!0)};
...
10}else fps=profiler.paintCountLog[profiler.paintCountLog.length-1];profiler.context.fillText("Browser FPS: "+fps,b,c+12)}profiler.context.restore()}};var Shadows=function(a){this.props={player:a.props}};
Shadows.prototype.draw=function(a){if(9<this.props.player.lastMoves.length){var b=-50*(this.props.player.z-this.props.player.minZ)/(this.props.player.maxZ-this.props.player.minZ),c=this.props.player.minZ,d=1.5*this.props.player.z-1.5*c;if(!(5>d)){var e=0+b,f=this.props.player.lastMoves[0][1]+d,g=this.props.player.lastMoves[6][0]+b,k=this.props.player.lastMoves[6][1]+d,h=this.props.player.lastMoves[7][0]+b,l=this.props.player.lastMoves[7][1]+d,b=100+this.props.player.lastMoves[9][0]+b,d=this.props.player.lastMoves[9][1]+
d;a.lineWidth=c;c=a.createLinearGradient(0,0,b-a.lineWidth/2,0);c.addColorStop(1,"rgba(36,42,48,0.1)");c.addColorStop(0,"rgba(36,42,48,0)");a.strokeStyle=c;a.lineCap="round";a.beginPath();a.moveTo(e,f);a.bezierCurveTo(g,k,h,l,b,d);a.stroke()}}};if("undefined"===typeof io)$(".main").hide(),$("#error").show(),$("#error-message").html("Can't connect to Multeor server");else if(!Modernizr.sessionstorage||!Modernizr.canvas||!Modernizr.websockets)$(".main").hide(),$("#error").show(),$("#error-message").html('Browser not supported<br />(<a href="/about/#requirements">read more</a>)');else{var canvas=document.getElementById("canvas"),context=canvas.getContext("2d");canvas.width=window.innerWidth;canvas.height=600;context.globalAlpha=0.5;var socket=
io.connect(window.location.hostname+":843"),players={},game=!1,playerColors="110,8,157 0,255,247 16,230,34 255,174,26 255,255,0 255,110,221 8,103,255 255,0,36".split(" "),playerColorsShuffled=shuffleArray(playerColors.slice(0)),gameRoom=sessionStorage.getItem("game-room")||Math.floor(1E4*Math.random()+1E4);sessionStorage.setItem("game-room",gameRoom);var viewerId=sessionStorage.getItem("viewer-id")||Math.floor(1E4*Math.random()+1E4);sessionStorage.setItem("viewer-id",viewerId);$(document).ready(function(){var a=
new Game("/levels/forest",gameRoom);socket.emit("new-viewer",{viewerId:viewerId,gameRoom:gameRoom});socket.on("game-end",function(b){a.abortGame()});socket.on("game-invalid",function(a){sessionStorage.setItem("viewer-id","");sessionStorage.setItem("game-room","");document.location="/";logGAEvent("Game hijacking?")});socket.on("update-game-state",function(a){var b=0,c;for(c in a.players)b++;b&&$("#gameStart").removeClass("disabled");b=0;for(c in a.players){var d=a.players[c].playerId;b++;if("undefined"==
typeof players[d]){var e=playerColorsShuffled.splice(0,1)[0];players[d]=new Player(context,d,a.players[c].playerIcon,e,b);socket.emit("update-player-color",{viewerId:viewerId,gameRoom:gameRoom,playerId:d,playerColor:e})}}for(var n in players){b=!1;for(c in a.players)a.players[c].playerId==n&&(b=!0);b||delete players[n]}});socket.on("player-update",function(a){if("undefined"==typeof players[a.pid])return!1;players[a.pid].props.vector=a.v});var b,c,d=(new Date).getTime(),e=1E3/60;(function g(k){requestAnimationFrame(g);
b=(new Date).getTime();c=b-d;c>e&&(d=b-c%e,a&&a.tick(context,c))})();$(".gamecode-container").html(gameRoom);$("#gameStart").off("click").on("click",function(b){b.preventDefault();if($(this).hasClass("disabled"))return!1;hideInstructions();a.getReady()});$("#gameReset button").off("click").on("click",function(b){b.preventDefault();a.resetGame()})})}
