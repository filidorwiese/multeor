...
Game.prototype.loadAudio=function(){var a=new Howl({urls:[this.props.levelPath+"/audio/level.mp3",this.props.levelPath+"/audio/level.ogg"],loop:!0});$(window).off("game-audio-start").on("game-audio-start",function(c){a.volume(0.5);a.play()});$(window).off("game-audio-stop").on("game-audio-stop",function(c){$({volume:1}).animate({volume:0},{easing:"linear",duration:5E3,step:function(c,b){a.volume(c)},complete:function(){a.stop()}})});for(var b in this.props.audio)this.props.audio[b]=new Howl({urls:[this.props.levelPath+
...
(this.props.state="LEADERBOARD",this.prepareLeaderboard());("ENDED"==this.props.state||"LEADERBOARD"==this.props.state)&&this.renderLeaderboard(a)};Game.prototype.renderBackgrounds=function(a,b,c,d){for(var e=0;e<=b;e++){var f=this.props.world[c+e];if("undefined"!=typeof f){var g=1E3*e-d;if(!(g>a.canvas.width)){var h=0,k=1E3;0>g&&(g=0,h=d,k=1E3-d);(f=this.getImage(f.background))&&a.drawImage(f,h,0,k,a.canvas.height,g,0,k,a.canvas.height)}}}};
Game.prototype.renderEntities=function(a,b,c,d){var e=[],f=0;this.props.spriteAnimationFrame=8.9>this.props.spriteAnimationFrame?this.props.spriteAnimationFrame+0.1:1;for(var g=Math.floor(this.props.spriteAnimationFrame),h=-2;h<=b;h++){var k=this.props.world[c+h];if("undefined"!=typeof k)for(var m in k.sprites){f++;var l=k.sprites[m],n=this.props.destroyed[l.id]||!1,q=1E3*h-d+l.left,s=l.top,t=50*l.layer,r=f+1E3*l.layer,u=this.getImage(l.path);q<-1*u.width||q>canvas.width||("STARTED"==this.props.state&&
(l.left-=1.25*(l.layer-1)),e[r]=new Destroyable(l,u,q,s,t,n,g),l.destroyable&&!n&&(n=e[r].collides(players,d),0<n&&(t=players[n].props.color,r=$.inArray(t,playerColors)+1,this.props.destroyed[l.id]=r,0<l.score&&(players[n].updateScore(l.score),r=f+1E3*l.layer+200,u=5*parseInt(l.layer,10),q=Explosion(q-50,s- -1*(players[n].props.z/2),r,u,t),this.props.explosions=this.props.explosions.concat(q)),l.audio&&"none"!=l.audio&&this.props.audio[l.audio].volume(1).play())))}}for(var p in players)b=1E3*players[p].props.vector[2]+
900+players[p].props.playerNumber,e[b]=players[p],b=players[p].props.playerNumber+1500,e[b]=new Shadows(players[p]);if(this.props.explosions.length)for(var v in this.props.explosions)p=this.props.explosions[v],p.dead||(e[p.zIndex]=p);e.sort(function(a,b){return a[1]-b[1]});for(v in e)e[v].draw(a,d)};Game.prototype.renderText=function(a){var b=a.canvas.width/2;this.drawMessage(a,this.props.message,(a.canvas.width-b)/2,a.canvas.height/2.2,b,35,!0)};
