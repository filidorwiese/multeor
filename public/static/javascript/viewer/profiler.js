...
profiler = {
	...
	start: function( context, target, x, y, graphScale ) {
		if( window.mozPaintCount ){ profiler.lastPaintCount = window.mozPaintCount; }
	...
	drawStats: function( average ) {
	    ...
	    if( average ){
	        var speed = 0;
	        for( i in profiler.speedLog ){ speed += profiler.speedLog[i]; }
	    ...
	    if( average ){
	        fps = 0;
	        for( i in profiler.fpsLog ){ fps += profiler.fpsLog[i]; }
	    ...
	    if( window.mozPaintCount ){     
	        ...
	        if( average ){
	            fps = 0;
	            for( i in profiler.paintCountLog ){ fps += profiler.paintCountLog[i]; }
